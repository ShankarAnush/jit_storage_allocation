from redis_q_agent import QAgent
from redis_storage_allocator import StorageAllocator
import time
import os
import numpy as np
import gym
...
def main():
    ...
    for e in range(episode_count + 1):
        ...
        for t in range(allocator.last_data_index):
            ...
            with open(r'redis_disk_space.csv', 'a') as f:
                writer = csv.writer(f)
            ...
            if done:
                ...
                print("-------------------------------------------------------------------------")
                with open(r'redis_downtime_reward.csv', 'a') as f:
                    writer = csv.writer(f)
    ...
    if not os.path.exists("models/redis_rl_storage_allocator.h5"):
        os.mkdir("models")
        agent.model.save("models/redis_rl_storage_allocator.h5")

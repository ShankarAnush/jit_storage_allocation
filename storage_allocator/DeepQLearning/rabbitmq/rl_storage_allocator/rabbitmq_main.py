...
def main():
    ...
    with open(r'rabbitmq_downtime_reward.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['rewards_per_episode', 'downtime_per_episode'])

    # Note disk_space every iteration
    with open(r'rabbitmq_disk_space.csv', 'w') as f:
        writer = csv.writer(f)
    ...
    for e in range(episode_count + 1):
        ...
        for t in range(allocator.last_data_index):
            ...
            with open(r'rabbitmq_disk_space.csv', 'a') as f:
                writer = csv.writer(f)
            ...
            if done:
                ...
                print("-------------------------------------------------------------------------")
                with open(r'rabbitmq_downtime_reward.csv', 'a') as f:
                    writer = csv.writer(f)
    ...
    if not os.path.exists("models/rabbitmq_rl_storage_allocator.h5"):
        os.mkdir("models")
        agent.model.save("models/rabbitmq_rl_storage_allocator.h5")

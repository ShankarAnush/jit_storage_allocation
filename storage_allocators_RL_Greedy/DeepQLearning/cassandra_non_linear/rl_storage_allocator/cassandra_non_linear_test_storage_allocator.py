...
def main():
    test_file = "cassandra_non_linear_testing_data"
    model_name = "cassandra_non_linear_rl_storage_allocator.h5"
    ...
    with open(r'rl_storage_cassandra_non_linear_downtime_reward.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['rewards', 'downtime'])

    with open(r'rl_storage_cassandra_non_linear_disk_space.csv', 'w') as f:
        writer = csv.writer(f)
    ...
    for t in range(storage_allocator.last_data_index):
        ...
        with open(r'rl_storage_cassandra_non_linear_disk_space.csv', 'a') as f:
            writer = csv.writer(f)
            writer.writerow([t, allocated_disk_space])

        with open(r'rl_storage_cassandra_non_linear_downtime_reward.csv', 'a') as f:
            writer = csv.writer(f)

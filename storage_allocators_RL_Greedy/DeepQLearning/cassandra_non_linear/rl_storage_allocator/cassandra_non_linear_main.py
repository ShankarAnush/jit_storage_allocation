from cassandra_non_linear_q_agent import QAgent
from cassandra_non_linear_storage_allocator import StorageAllocator
import time
import os
import numpy as np
import gym
...
def main():
    window_size = 10
    episode_count = 100
    df = "cassandra_non_linear_testing_data"
    batch_size = 32
    ...
    with open(r'cassandra_non_linear_downtime_reward.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['rewards_per_episode', 'downtime_per_episode'])

    # Note disk_space every iteration
    with open(r'cassandra_non_linear_disk_space.csv', 'w') as f:
        writer = csv.writer(f)
    ...
    for e in range(episode_count + 1):
        ...
        for t in range(allocator.last_data_index):
            ...
            with open(r'cassandra_non_linear_disk_space.csv', 'a') as f:
                writer = csv.writer(f)
            ...
            if done:
                ...
                print("-------------------------------------------------------------------------")
                with open(r'cassandra_non_linear_downtime_reward.csv', 'a') as f:
                    writer = csv.writer(f)
    ...
    if not os.path.exists("models/cassandra_non_linear_rl_storage_allocator.h5"):
        os.mkdir("models")
        agent.model.save("models/cassandra_non_linear_rl_storage_allocator.h5")
